...
if($RIGHT >= "R"){
					...
					"LINK" => array(
						...
						"ITEMS" => array(
							...
							"LEAD_NAME_AMO_CRM_TITLE" => array(
								"TYPE" => "text",
								"VALUE" => Option::get($moduleID, "LEAD_NAME_AMO_CRM_TITLE", Loc::getMessage("ASPRO_ALLCORP3_MODULE_LEAD_NAME_AMO_CRM"), $optionsSiteID),
							),
							"TAGS_AMO_CRM_TITLE" => array(
								"TYPE" => "text",
								"VALUE" => Option::get($moduleID, "TAGS_AMO_CRM_TITLE", Loc::getMessage("ASPRO_ALLCORP3_MODULE_TAGS_AMO_CRM"), $optionsSiteID),
							),
	...
	if($REQUEST_METHOD == "POST" && strlen($Update.$Apply.$RestoreDefaults.$action) && $RIGHT >= "W" && check_bitrix_sessid()){
		...
		if($action === 'checkAuth'){
			...
			if(strlen($arConfig['domain'])){
				// check auth
				$arResponse = CAsproAllcorp3CRM::checkOAuth($arOAuth, $arConfig);
				if(
					$arResponse &&
					is_array($arResponse) &&
					!$arResponse['error']
					...
					CAsproAllcorp3CRM::save($arOAuth, $arConfig);
				}
			}
			else{
				$arResponse = array(
		...
		elseif($action === 'sendCrm'){
			...
			if(
				...
				echo \Aspro\Functions\CAsproAllcorp3::sendLeadCrmFromForm($formId, $resultId, 'AMO_CRM', $siteId, false, false);
			}
			else{
				$arResponse = array(
					'error' => 'empty fields',
	...
	<?if(!count($arTabs)):?>
		<div class="adm-info-message-wrap adm-info-message-red">
			...
			<a href="aspro.allcorp3_options_tabs.php" id="tabs_settings" target="_blank">
				<span>
								...
								<?if($arOption["VALUES"]):?>
									<tr>
										<td colspan="2">
											...
											if($bOAuthChecked){
												$url = str_replace('#DOMAIN#', Option::get($moduleID, 'DOMAIN_AMO_CRM', '', $optionsSiteID), \Aspro\Functions\CAsproAllcorp3CRM::AMO_CRM_PATH);
												$result_text = \Aspro\Functions\CAsproAllcorp3CRM::query($url, "/private/api/v2/json/accounts/current/", array(), $arHeaders, false, true);
															...
															if($codeGroup != "customers")
															{
																foreach($arGroup as $arProp)
																{
																	...
																	if(isset($arProp["enums"]))
																	{
																		...
																		{
																			\Aspro\Functions\CAsproAllcorp3CRM::$arCrmFileds["AMO_CRM"][$codeGroup]["PROPS"][$arProp["id"]."_".$keyEnum."_".$codeGroup] = (Loc::getMessage("AMO_CRM_FIELD_".$arProp["code"]."_".$enum) ? Loc::getMessage("AMO_CRM_FIELD_".$arProp["code"]."_".$enum) : $arProp["name"]."_".$enum);
																		}
																	}
																	else
																	{
																		\Aspro\Functions\CAsproAllcorp3CRM::$arCrmFileds["AMO_CRM"][$codeGroup]["PROPS"][$arProp["id"]."_".$codeGroup] = $arProp["name"];
																	}
																}
															}
														}
																				...
																				<tr>
																					<td class="adm-detail-content-cell-l" style="width:50%;">
																						<select name="CRM_FIELD_<?=$optionsSiteID?>[<?=$key;?>][]" class="field_crm" style="width:300px;">
																							...
																							<?foreach(\Aspro\Functions\CAsproAllcorp3CRM::$arCrmFileds["AMO_CRM"] as $groupCode => $arGroup):?>
																					...
																					<td style="width:50%;">
																						<select name="CRM_FORM_FIELD_<?=$optionsSiteID?>[<?=$key;?>][]" class="field_form" style="width:300px;">
																							<option value=""><?=Loc::getMessage('FORM_FIELD_CRM_FIELDS_FORM_NAME_NO')?></option>
																							<optgroup label="...">
																								<?foreach(\Aspro\Functions\CAsproAllcorp3CRM::$arCrmFileds["MAIN"] as $key2 => $text):?>
