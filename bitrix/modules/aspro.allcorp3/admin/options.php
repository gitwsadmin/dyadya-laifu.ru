...
$moduleAdminClass = "\Aspro\Functions\CAsproAllcorp3Admin";
...
if($RIGHT >= "R"){
	...
	$arWebFormsOptions = array(
		'init' => $moduleClass::$arParametrsList['WEB_FORMS']['OPTIONS'],
	);
	...
	foreach($arSites as $key => $arSite){
		if(
			...
			$arWebFormsOptions[$arSite["ID"]] = $moduleClass::$arParametrsList['WEB_FORMS']['OPTIONS'];
			// restore initial webforms options
			$moduleClass::$arParametrsList['WEB_FORMS']['OPTIONS'] = $arWebFormsOptions['init'];
			...
			$arTabs[] = array(
				...
				"SITE_ID" => $arSite["ID"],
				"SITE_DIR" => $arSite["DIR"],
				"TEMPLATE" => $moduleClass::GetSiteTemplate($arSite["ID"]),
	...
	if($REQUEST_METHOD == "POST" && strlen($Update.$Apply.$RestoreDefaults) > 0 && $RIGHT >= "W" && check_bitrix_sessid()){
		...
		if(strlen($RestoreDefaults) > 0){
			Option::delete($moduleClass::moduleID);
			Option::delete($moduleClass::moduleID, array("name" => "NeedGenerateCustomTheme"));
			Option::delete($moduleClass::moduleID, array("name" => "NeedGenerateCustomThemeBG"));
			$APPLICATION->DelGroupRight($moduleClass::moduleID);
		}
		else{
			Option::delete($moduleClass::moduleID, array("name" => "sid"));
			...
			foreach($arTabs as $key => $arTab){
				...
				$moduleClass::$arParametrsList['WEB_FORMS']['OPTIONS'] = $arWebFormsOptions[$optionsSiteID];
				...
				CBitrixComponent::clearComponentCache('aspro:com.banners.allcorp3', $optionsSiteID);
				CBitrixComponent::clearComponentCache('aspro:catalog.section.list.allcorp3', $optionsSiteID);
				CBitrixComponent::clearComponentCache('aspro:social.info.allcorp3', $optionsSiteID);
				$arTabs[$key] = $arTab;
			}
		}
		...
		// send statistics
		if(\Aspro\Allcorp3\GS::isEnabled()){
			if(\Aspro\Allcorp3\GS::register()){
				\Aspro\Allcorp3\GS::sendData(
					\Aspro\Allcorp3\GS::mkData(array('sites', 'options'))
				);
			}
		}
	...
	<?if(!count($arTabs)):?>
		<div class="adm-info-message-wrap adm-info-message-red">
			...
			<a href="aspro.allcorp3_options_tabs.php?lang=<?=LANGUAGE_ID?>" id="tabs_settings" target="_blank">
				<span>
	...
	<?else:?>
		...
		$context = new CAdminContextMenu(
			array(
				array(
					...
					'LINK' => 'aspro.allcorp3_options_tabs.php?lang='.LANGUAGE_ID,
				)
			)
		);
		$context->Show();
		...
		$moduleClass::$arParametrsList["SECTION"]["OPTIONS"]["SECTION_CONTACTS_GROUP4"]["ONE_BLOCK"] = "Y";
		...
		$arRequiredOptions = array(
			...
			"SECTION_CONTACTS_GROUP4" => $moduleClass::$arParametrsList["SECTION"]["OPTIONS"]["SECTION_CONTACTS_GROUP4"],
		...
		unset($moduleClass::$arParametrsList["SECTION"]["OPTIONS"]["SECTION_CONTACTS_GROUP4"]);
		...
		foreach($arTabs as $key => $arTab)
		{
			...
			if($arTab["SITE_ID"])
			{
				...
				// restore webforms options for site
				$moduleClass::$arParametrsList['WEB_FORMS']['OPTIONS'] = array();
				foreach($arWebFormsOptions[$optionsSiteID] as $optionCode => $arOption){
					if(strpos($optionCode, 'EXPRESS_BUTTON') !== false){
						if(!isset($moduleClass::$arParametrsList['WEB_FORMS']['OPTIONS']['EXPRESS_BUTTON_GROUP'])){
							$moduleClass::$arParametrsList['WEB_FORMS']['OPTIONS']['EXPRESS_BUTTON_GROUP'] = array(
						...
						$moduleClass::$arParametrsList['WEB_FORMS']['OPTIONS']['EXPRESS_BUTTON_GROUP']['ITEMS'][$optionCode] = $arOption;
					}
					elseif( strpos($optionCode, 'ASPRO_ALLCORP3_') !== false ){
						if(!isset($moduleClass::$arParametrsList['WEB_FORMS']['OPTIONS']['FORMS_OPTIONS_GROUP'])){
							$moduleClass::$arParametrsList['WEB_FORMS']['OPTIONS']['FORMS_OPTIONS_GROUP'] = array(
								'TITLE' => GetMessage('FORMS_OPTIONS_GROUP'),
								'ONE_BLOCK' => 'N',
								'ITEMS' => array()
							);
						}
						$moduleClass::$arParametrsList['WEB_FORMS']['OPTIONS']['FORMS_OPTIONS_GROUP']['ITEMS'][$optionCode] = $arOption;
					}
					else{
						$moduleClass::$arParametrsList['WEB_FORMS']['OPTIONS'][$optionCode] = $arOption;
					}
				}
				?>
				<tr>
									...
									<?foreach($moduleClass::$arParametrsList as $blockCode => $arBlock){?>
										...
										<div class="tab <?=($bTabActive ? "active" : "")?>" data-prop_code="<?=$blockCode?>">
											...
											<?foreach($arBlock["OPTIONS"] as $optionCode => $arOption){?>
												...
												<?if(isset($arOption["ITEMS"])):?>
													...
													<div class="groups_block block <?=$optionCode?> <?=(isset($arOption["PARENT"]) ? "depend-block" : "");?>" <?=(isset($arOption["PARENT"]) ? "data-parent='".$arOption["PARENT"]."_".$optionsSiteID."'" : "");?> <?=$style;?>>
														...
														<?foreach($arOption["ITEMS"] as $optionCode2 => $arOption2):?>
															<?=$moduleClass::showAllAdminRows($optionCode2, $arTab, $arOption2, $module_id, $arPersonTypes, $optionsSiteID, $arDeliveryServices, $arPaySystems, $arCurrency, $arOrderPropertiesByPerson, $bSearchMode);?>
														<?endforeach;?>
												...
												<?else:?>
													<div class="block">
														<?=$moduleClass::showAllAdminRows($optionCode, $arTab, $arOption, $module_id, $arPersonTypes, $optionsSiteID, $arDeliveryServices, $arPaySystems, $arCurrency, $arOrderPropertiesByPerson, $bSearchMode);?>
													</div>
												<?endif;?>
											<?}?>
										</div>
			...
			<script type="text/javascript">
				...
				$(document).ready(function() {
					...
					$('.adm-btn[data-action=exportToFile],.adm-btn[data-action=importFromLink],.adm-btn[data-action=importFromFile]').click(function(e){
						...
						var componentName = '<?=$moduleClass::partnerName?>:theme.<?=$moduleClass::solutionName?>';
