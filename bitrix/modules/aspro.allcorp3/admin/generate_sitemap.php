...
if($RIGHT >= "R"){
	...
	foreach($arSites as $key => $arSite){
		...
		$bGenerate = ((Option::get("aspro.allcorp3", "REGIONALITY_TYPE", "ONE_DOMAIN", $optionsSiteID) == "SUBDOMAIN") && Option::get("aspro.allcorp3", "USE_REGIONALITY", "N", $optionsSiteID) == "Y");
		if($bGenerate)
			$bShowGenerate = true;
			...
			foreach($arTabs as $key => $arTab)
			{
				...
				if(isset($arTab["OPTIONS"]) && $arTab["OPTIONS"])
				{
					foreach($arTab["OPTIONS"] as $optionCode => $arOption)
					{
						...
						if($_POST[$optionCode."_".$optionsSiteID])
						{
							Option::set("aspro.allcorp3", $optionCode, $_POST[$optionCode."_".$optionsSiteID], $optionsSiteID);							
						}							
					}
					$siteMapName =  Option::get("aspro.allcorp3", "SITEMAP_NAME", $arTab["OPTIONS"]["SITEMAP_NAME"]["DEFAULT"], $optionsSiteID);
					$siteMapNewName =  Option::get("aspro.allcorp3", "SITEMAP_NEW_NAME", $arTab["OPTIONS"]["SITEMAP_NEW_NAME"]["DEFAULT"], $optionsSiteID);
					$siteMapUrl =  Option::get("aspro.allcorp3", "SITEMAP_URL", $arTab["OPTIONS"]["SITEMAP_URL"]["DEFAULT"], $optionsSiteID);
		...
		foreach($arTabs as $key => $arTab)
		{
			...
			if($arTab["SITE_ID"])
			{
				...
				<?if(isset($arTab["OPTIONS"]) && $arTab["OPTIONS"]):?>
					...
					<?foreach($arTab["OPTIONS"] as $optionCode => $arOption):?>
						<?$val = Option::get("aspro.allcorp3", $optionCode, $arOption["DEFAULT"], $optionsSiteID);?>
						<tr>
							<td>
								<?=$arOption["TITLE"];?>							
							</td>
