...
if($RIGHT >= "R")
{
						...
						"LINK" => array(
							...
							"ITEMS" => array(
								...
								"LEAD_NAME_FLOWLU_TITLE" => array(
									"TYPE" => "text",
									"VALUE" => Option::get($moduleID, "LEAD_NAME_FLOWLU_TITLE", Loc::getMessage("ASPRO_ALLCORP3_MODULE_LEAD_NAME_FLOWLU"), $optionsSiteID),
								),
	...
	if($REQUEST_METHOD == "POST" && strlen($Update.$Apply.$RestoreDefaults.$Auth.$SendCrm) && $RIGHT >= "W" && check_bitrix_sessid())
	{
		...
		if($Auth)
		{
			if($_POST["DOMAIN"] && $_POST["TOKEN"])
			{
				if(count(explode('.', $_POST["DOMAIN"])) > 2){
					$url = str_replace("#DOMAIN#", $_POST["DOMAIN"], \Aspro\Functions\CAsproAllcorp3CRM::FLOWLU_PATH);
				}
				else{
					$url = str_replace("#DOMAIN#", $_POST["DOMAIN"], \Aspro\Functions\CAsproAllcorp3CRM::FLOWLU_SHORT_PATH);
				}
				...
				echo \Aspro\Functions\CAsproAllcorp3CRM::query(
					$url,
					\Aspro\Functions\CAsproAllcorp3CRM::$arCrmMethods['FLOWLU']['LIST_LEADS'],
					$arPostFields,
					array()
				);
			}
		...
		elseif($SendCrm)
		{
			...
			{
				echo \Aspro\Functions\CAsproAllcorp3::sendLeadCrmFromForm($_POST["FORM_ID"], $_POST["RESULT_ID"], 'FLOWLU', $_POST["SITE_ID"]);;
			}
			else
			{
				$arError = array(
			...
			foreach($arTabs as $key => $arTab)
			{
				...
				if($domain && $token){
					if(count(explode('.', $domain)) > 2){
						$url = str_replace("#DOMAIN#", $domain, \Aspro\Functions\CAsproAllcorp3CRM::FLOWLU_PATH);
					}
					else{
						$url = str_replace("#DOMAIN#", $domain, \Aspro\Functions\CAsproAllcorp3CRM::FLOWLU_SHORT_PATH);
					}
					...
					$result = json_decode(
						\Aspro\Functions\CAsproAllcorp3CRM::query(
							$url,
							\Aspro\Functions\CAsproAllcorp3CRM::$arCrmMethods['FLOWLU']['LIST_LEADS'],
							$arPostFields,
							array()
																					...
																					<td class="adm-detail-content-cell-l" style="width:50%;">
																						<select name="CRM_FIELD_<?=$optionsSiteID?>[<?=$key;?>][]" class="field_crm" style="width:300px;">
																							...
																							<optgroup label="<?=Loc::getMessage('FORM_FIELD_CRM_FIELDS_LEAD')?>">
																								<?foreach(\Aspro\Functions\CAsproAllcorp3CRM::$arCrmFileds["FLOWLU"] as $key2 => $text):?>
