<?
// Cache functions
if(!class_exists('CAllcorp3Cache')){
	class CAllcorp3Cache {
		...
		public static function CSaleBasket_GetList($arOrder = array("SORT" => "ASC"), $arFilter = array(), $arGroupBy = false, $arNavStartParams = false, $arSelectFields = array(), $cacheTag = "", $cacheTime = 36000000, $cachePath = ""){
			...
			if(!strlen($cachePath)){
				$cachePath = "/CAllcorp3Cache/sale/CSaleBasket_GetList/".$cacheTag."/";
			}
			$obCache = new CPHPCache();
		...
		public static function CCatalogStore_GetList($arOrder = array("SORT" => "ASC"), $arFilter = array(), $arGroupBy = false, $arNavStartParams = false, $arSelectFields = array(), $cacheTag = "", $cacheTime = 36000000, $cachePath = ""){
			...
			if(!strlen($cachePath)){
				$cachePath = "/CAllcorp3Cache/catalog/CCatalogStore_GetList/".$cacheTag."/";
			}
			$obCache = new CPHPCache();
		...
		public static function CForumMessage_GetListEx($arOrder = array("SORT" => "ASC"), $arFilter = array(), $arGroupBy = false, $iNum = 0, $arSelectFields = array(), $cacheTag = "", $cacheTime = 36000000, $cachePath = ""){
			...
			if(!strlen($cachePath)){
				$cachePath = "/CAllcorp3Cache/forum/CForumMessage_GetListEx/".$cacheTag."/";
			}
			$obCache = new CPHPCache();
		...
		private static function _InitCacheParams($moduleName, $functionName, $arCache){
			...
			if(!strlen($cachePath)){
				$cachePath = "/CAllcorp3Cache/".$moduleName."/".$functionName."/".$cacheTag."/";
			}
	...
	if(CAllcorp3Cache::$arIBlocks === NULL){
		CAllcorp3Cache::$bEnabled = COption::GetOptionString('main', 'component_managed_cache_on', 'Y') === 'Y';
		$arIBlocksTmp = CAllcorp3Cache::CIBlock_GetList(array("CACHE" => array("TAG" => "iblocks")), array("ACTIVE" => "Y", "CHECK_PERMISSIONS" => "N"));
		CAllcorp3Cache::$arIBlocks = CAllcorp3Cache::GroupArrayBy($arIBlocksTmp, array("GROUP" => array("LID", "IBLOCK_TYPE_ID", "CODE"), "MULTI" => "Y", "RESULT" => array("ID")));
		CAllcorp3Cache::$arIBlocksInfo = CAllcorp3Cache::GroupArrayBy($arIBlocksTmp, array("GROUP" => array("ID")));
	}
}