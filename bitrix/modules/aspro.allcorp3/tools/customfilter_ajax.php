...
if($action){
	if($action === 'init' || $action === 'save'){
		...
		$eventManager->addEventHandlerCompatible(
			...
			array(
				'\Aspro\Allcorp3\CCatalogCondCtrlGroup',
				'GetControlDescr'
			)
		);
		...
		$eventManager->addEventHandlerCompatible(
			...
			'OnCondCatControlBuildList', 
			array(
				'\Aspro\Allcorp3\CCatalogCondCtrlIBlockFields',
				'GetControlDescr'
			)
		);
		...
		$eventManager->addEventHandlerCompatible(
			...
			'OnCondCatControlBuildList', 
			array(
				'\Aspro\Allcorp3\CCatalogCondCtrlIBlockProps',
				'GetControlDescr'
			)
		);
		...
		if(!empty($ids) && is_array($ids)){
			$condTree = new Aspro\Allcorp3\CCatalogCondTree();
		...
		if(check_bitrix_sessid() && $request->isPost()){
			...
			elseif($action === 'get_crosssales_iblockprops'){
				...
				if($propertyId > 0){
					...
					if($property){
						$arExcludeUserTypes = \Aspro\Allcorp3\Property\CustomFilter\CondCtrl::getCrossSalesExcludePropertyUserTypes();
