<?
namespace Aspro\Allcorp3\Property;
...
class ListWebForms{
	static function OnIBlockPropertyBuildList(){
		return array(
			'PROPERTY_TYPE' => 'S',
			'USER_TYPE' => 'SAsproAllcorp3ListWebForms',
			'DESCRIPTION' => Loc::getMessage('WEBFORMS_LINK_PROP_ALLCORP3_TITLE'),
	...
	protected static function _getSites($arProperty){
		return \CAllcorp3Cache::$arIBlocksInfo[$arProperty['IBLOCK_ID']] ? \CAllcorp3Cache::$arIBlocksInfo[$arProperty['IBLOCK_ID']]['LID'] : array();
	}
	...
	static function getWebForms($arSites){
		...
		if(Loader::includeModule('iblock')){
			foreach(\CAllcorp3Cache::$arIBlocks as $siteId => $arSiteIBlocks){
				if(!$arSites || in_array($siteId, $arSites)){
					foreach(\CAllcorp3Cache::$arIBlocks[$siteId]['aspro_allcorp3_form'] as $arIblocks){
						foreach($arIblocks as $iblockId){
							$arResult['IBLOCK'][\CAllcorp3Cache::$arIBlocksInfo[$iblockId]['CODE']] = \CAllcorp3Cache::$arIBlocksInfo[$iblockId]['NAME'];
						}
					}
				}
			}
