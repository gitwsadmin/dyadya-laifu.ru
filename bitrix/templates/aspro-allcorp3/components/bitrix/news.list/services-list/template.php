...
if ($currentPage === "/") :?>
    ...
    <div class="mainpage-blocks ws-index-services-wrapper">
        <div class="maxwidth-theme">
            ...
            <div class="ws-index-services">

                <?php foreach ($arItems as $i => $arItem): ?>
                    <a class="ws-index-services__item" href="<?=$arItem['DETAIL_PAGE_URL']?>">
                        <div class="ws-index-services__image">
                            <img src="<?=$arItem['PREVIEW_PICTURE']['SRC']?>"
                                 alt="<?=$arItem['NAME']?>">
                        </div>
                        <div class="ws-index-services__title">
                            <?=$arItem['NAME']?>
                        </div>
                    </a>
                <?php endforeach; ?>
...
<?php else : ?>
    ...
    <? if ($arItems): ?>
        ...
        foreach ($arItems as $i => $arItem):?>
            ...
            // use detail link?
            $bDetailLink = $arParams['SHOW_DETAIL_LINK'] != 'N' && (!strlen(
                    $arItem['DETAIL_TEXT']
            ...
            $bShowPrice = $bItemsTypeElements && in_array('PRICE', $arParams['PROPERTY_CODE']) && strlen(
                                ...
                                ?>
                                    <? if ($bIcons && $nImageID):?>
                                        <?=CAllcorp3::showIconSvg(
                                            ' fill-theme services-list__item-image-icon',
                                            $imageSrc
                                        );?>
                                    <? else:?>
                                ...
                                <? if ($bDetailLink):?>
                                    <a class="arrow-all stroke-theme-target" href="<?=$detailUrl?>">
                                        <?=CAllcorp3::showIconSvg(
                                            ...
                                            SITE_TEMPLATE_PATH . '/images/svg/Arrow_map.svg'
                                        );?>
                        ...
                        <div class="services-list__item-additional-text-wrapper">
                            <div class="services-list__item-additional-text-top-part">
                                <? if ($arItem['SECTIONS'] && $arParams['SHOW_SECTION'] != 'N'):?>
                                    <div class="services-list__item-section font_13 color_light--opacity"><?=implode(
                                            ', ',
                                            $arItem['SECTIONS']
                                        )?></div>
                                <?endif; ?>
                        ...
                        <div class="flexbox <?=($bShowImage && $imageSrc ? 'maxwidth-theme--half flex-1' : 'maxwidth-theme')?>"><?endif;
                            ...
                            <div class="services-list__item-text-top-part <?=($bFonImg ? 'srollbar-custom scroll-deferred' : '')?> <?=($bSRLImg && !$arParams['NARROW'] && !($bShowImage && $imageSrc)) ? 'flex-1' : ''?>">
                                <? if ($bItemsTypeElements && $arItem['SECTIONS'] && $arParams['SHOW_SECTION'] != 'N'):?>
                                    <div class="services-list__item-section font_13 <?=($bFonImg ? 'color_light--opacity' : 'color_999')?>"><?=implode(
                                            ', ',
                                            $arItem['SECTIONS']
                                        )?></div>
                                <?endif; ?>
                                            ...
                                            <? if ($arParams['ELEMENTS_ROW'] == 1):?>
                                                <a class="arrow-all arrow-all--wide stroke-theme-target"
                                                   href="<?=$detailUrl?>">
                                                    <?=CAllcorp3::showIconSvg(
                                                        ' arrow-all__item-arrow',
                                                        SITE_TEMPLATE_PATH . '/images/svg/Arrow_lg.svg'
                                                    );?>
                                                    <div class="arrow-all__item-line colored_theme_hover_bg-el"></div>
                                                </a>
                                            <? else:?>
                                                    ...
                                                    <div class="services-list__item-properties-item-value">
                                                        <? if ($PCODE === 'SITE'):?>
                                                            ...
                                                            <a href="<?=(strpos(
                                                                $arProperty['VALUE'],
                                                                'http'
                                                            ) === false ? 'http://' : '') . $arProperty['VALUE'];?>"
                                                               rel="nofollow" target="_blank" class="dark-link">
                                                                <?=strpos(
                                                                    $arProperty['VALUE'],
                                                                    '?'
                                                                ) === false ? $arProperty['VALUE'] : explode(
                                                                                                         '?',
                                                                                                         $arProperty['VALUE']
                                                                                                     )[0]?>
                                                            </a>
                                                ...
                                                <li class="services-list__item-childs-item-wraper">
                                                    ...
                                                    if (is_array($arChild['DETAIL_PAGE_URL'])) {
                                                        if (isset($arChild['CANONICAL_PAGE_URL']) && !empty($arChild['CANONICAL_PAGE_URL'])) {
                                                            ...
                                                            $arChild['DETAIL_PAGE_URL'] = $arChild['DETAIL_PAGE_URL'][key(
                                                                $arChild['DETAIL_PAGE_URL']
                                                            )];
                                                        }
                                                    }
                                                    ?>
                                                    <? $bShowChildDetail = $arChild['SECTION_PAGE_URL'] || ($arParams['SHOW_ELEMENTS_DETAIL_LINK'] != 'N' && (!strlen(
                                                                $arChild['DETAIL_TEXT']
                                                            ) ? ($arParams['HIDE_ELEMENTS_LINK_WHEN_NO_DETAIL'] !== 'Y' && $arParams['HIDE_ELEMENTS_LINK_WHEN_NO_DETAIL'] != 1) : true)); ?>
                                                    <? if ($bShowChildDetail): ?>
                            ...
                            <? if ($bShowBottom):?>
                                <div class="services-list__item-text-bottom-part <?=($bShowPrice ? 'services-list__item-text-bottom-part--has-price' : '')?>">
                                    ...
                                    <? if ($arParams['IMAGE_POSITION'] === 'RIGHT' || $arParams['IMAGE_POSITION'] === 'SIDE'):?>
                                        <? if ($bDetailLink):?>
                                            <a class="btn btn-default btn-lg" href="<?=$detailUrl?>"><?=Loc::getMessage(
                                                    'SHOW_MORE'
                                                )?></a>
                                        <?endif; ?>
                                    <?endif; ?>
